...
#[test]
fn plan_week_help_explains_anchor_and_offset_inputs() {
    let output = mealie(&["plan", "week", "--help"]);

    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).expect("utf8 stdout");
    assert!(stdout.contains("Show a Monday-to-Sunday meal-plan view"));
    assert!(stdout.contains("--date <DATE>"));
    assert!(stdout.contains("--offset <WEEKS>"));
    assert!(stdout.contains("--date and --offset cannot be used together."));
    assert!(stdout.contains("mealie plan week --offset -1"));
}
